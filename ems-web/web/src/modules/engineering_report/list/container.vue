<template>
  <div>
    <UI
      :config="config"
      :tableData="records"
      @addWeek="handleAdd.call(this, 'EngineeringReportWeekAdd')"
    ...
    />
    <Create ref="create" :engineeringId="engineeringId" />
  </div>
</template>
...
import Create from '@/modules/engineering_report_week/create';
...
export default {
  ...
  computed: {
    ...mapState('engineeringReport', [
      'records',
      'total',
    ]),
  },
  methods: {
    ...mapActions('engineeringReport', [
      'getRecords',
    ]),
    init() {
      this.getRecords({
