<template>
  <div class="list-wrapper">
    <div class="list-top">
      ...
      <!-- <Button @click="$emit('addSchedule')">新建月度形象进度表</Button> -->
    </div>
    <div class="list">
        ...
        >
          <template slot="state" slot-scope="scope">
            <span :class="[$style.state, { 'is-gray': scope.data.state === '1' }]">
              {{ config.stateMap[scope.data.state] || config.stateMap.default }}
            </span>
          </template>
          <template slot="operate" slot-scope="scope">
            <span class="list-operate">
              <Button @click="$emit('edit', scope)" v-if="scope.data.state !== '1'">编辑</Button>
            </span>
          </template>
        </Table>
      </section>
