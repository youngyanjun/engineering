...
export default {
  ...
  methods: {
    ...
    initHelp() {
      this.getNextProgress({ weekly: this.baseInfo.weeklyNum - 1, engineeringId: this.engineeringId }).then(data => {
        data = data || {};
        // 字段转对象
        Object.keys(data).forEach(k => {
          if (k !== 'id' && k !== 'machine' && k !== 'personnel' && k !== 'stageInfoRemark') {
      ...
      this.getNextPerson({ weekly: this.baseInfo.weeklyNum - 1, engineeringId: this.engineeringId }).then(data => {
        const personnel = this.config.personnelOption.map(k => {
          const temp = {};
          temp.person = k.label;
          temp.api = k.value;
      ...
      this.getNextMachine({ weekly: this.baseInfo.weeklyNum - 1, engineeringId: this.engineeringId }).then(data => {
        const machine = this.config.machineOption.map(k => {
          const temp = {};
          temp.person = k.label;
          temp.api = k.value;
